...
class PrimemenContent extends Component {
  ...
  fetchContent = async () => {
    try {
      ...
      console.log('response of fetchContent => ', response);
  ...
  onImageChange = (event) =>  {
    ...
    if(event.target.files && event.target.files[0]) {
        ...
        reader.onload = (e) => {
            ...
            console.log(primemenContentArr[target.dataset.id][target.name]);
            primemenContentArr[target.dataset.id][target.name] = [...primemenContentArr[target.dataset.id][target.name], e.target.result];
  ...
  addContent = async () => {
    try {
      let arr = this.state.primemenContentArr.filter(obj => (!obj.title.trim() || !obj.description.trim()) );
      if(!arr.length) {
        let reqPayload = {
          name: "Primemen",
