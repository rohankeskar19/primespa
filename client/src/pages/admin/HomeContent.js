...
class HomeContent extends Component {
  ...
  fetchHomeContent = async () => {
    try {
      ...
      console.log('response of  fetchHomeContent => ', response);
  ...
  onImageChange = (event) =>  {
    ...
    if(event.target.files && event.target.files[0]) {
        ...
        reader.onload = (e) => {
            ...
            console.log(homeContentArr[target.dataset.id][target.name]);
            homeContentArr[target.dataset.id][target.name] = [...homeContentArr[target.dataset.id][target.name], e.target.result];
            this.setState({ homeContentArr }, () => console.log(this.state.homeContentArr));
        }
  ...
  addHomeContent = async () => {
    try {
      let arr = this.state.homeContentArr.filter(obj => (!obj.title.trim() || !obj.description.trim()) );
      if(!arr.length) {
        let reqPayload = {
          name: "Home",
